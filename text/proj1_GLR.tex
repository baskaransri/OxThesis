...
\mccorrect{Despite the practical importance of GLR reconstruction, especially in the high noise and large graph settings -- for example, in the Drug-Target interaction literature} \cite{chen2018machine}, \mccorrect{much work is based on a variant of GLR called LapRLS} \cite{belkin2006manifold} --  theoretical results on GLR in the GSP literature on the impact of sample size on reconstruction error are nearly non-existent (see Chapter \ref{ch:2-litreview}, Table \ref{tbl:Lit_review}). \mccorrect{We now briefly outline the strategies used in the GSP literature to prove theoretical results for LS, and their applicability or inapplicability to GLR. Firstly, approaches centered around perfect reconstruction of signals, such as} \cite{shomorony2014sampling}, \mccorrect{struggle because they utilise the fact that LS projects into a low dimensional subspace, and use this subspace dimension to bound sample size; GLR is not a subspace projection method and thus does not have this property. Secondly, approaches based on Bayesian-Optimal reconstruction are plausible; they would need to consider a} $\mathcal{N}(0,\matr{L}^{\dagger})$-distributed \mccorrect{signal, and if there was noise they would need to study LapRLS instead of GLR, both of which are uncommon in GSP. We do not take this approach as it limits the scope of our work by strictly limiting the signal model and taking the theoretical work further away from how GLR reconstruction is currently used in the GSP literature. A theoretical approach based around determinental sampling is also possible, but graph interpolation work in that area focuses on variants of GLR} \cite[(4.29)]{tremblay2024graph}, \mccorrect{and such an approach would be limited to one kind of sampling. We finally address the one major theoretical result on GLR, which comes from the learning theory literature. Belkin et al.} \cite{belkin2006manifold} \mccorrect{study a variant of GLR, which restricts the reconstructed signal} $\vect{x}$ to have the property $\vect{1}^{T}\vect{x} = 0$, \mccorrect{for stability reasons. They produce bounds for the generalisation error as sample size varies. While this approach can likely be extended to the MMSE setting under the constraint that} $\vect{1}^{T}\vect{x}$\mccorrect{, we believe that the constraint given is onerous and does not reflect common use, and it is not obvious how to extend this approach to the unrestricted case. This lack of immediate generalisation is because the high level theorem by Bousquet \& Elisseef that this result is based on} \cite[Theorem 4]{belkin2004regularization} requires $\beta$-stability, \mccorrect{which has been obtained through the constraint.}

In this chapter, we provide a theoretical characterisation of the impact of sample size on MMSE across the full range of sample sizes and the full range of noise levels for GLR reconstruction, across \mccorrect{all possible sample set selection strategies}. We focus on whether under sufficiently low SNRs, decreasing sample size may actually decrease MSE, which is important for the application of GLR reconstruction, a robust method, to domains where the noise may be greater than the signal. Like in the previous Chapter, this breadth is only possible through our rigorous theoretical characterisations which allow us to understand behaviour at high noise levels, to characterise behaviour on arbitrarily large graphs and to show when certain behaviours of MSE happen and why.
