...
\end{equation}
For each graph, we construct $f_{sgc}$ as a linearized GCN with randomly initialized weights and added self-loops ($\gamma=1)$. We choose our architecture to mimic the strong baselines in \cite{luo2024classic}, but keep to a single convolutional layer to minimise oversmoothing.
Specifically, we set
\begin{equation}
 f_{sgc}(\matr{X}) = \tilde{\matr{A}}_{1}\matr{X}\left(\matr{W}_{1}\matr{W}_{2} \right) \label{eq:SGC_structure}
\end{equation}
...
\begin{figure*}%
    ...
    \end{subfigure}%
    \caption{ $f_{simple}$, LS Reconstruction, BA graph model, noiseless}
...
\begin{figure*}%
    ...
    \end{subfigure}%
    \caption{$f_{sgc}$, LS Reconstruction, BA graph model, noiseless}
...
\begin{figure*}%
    ...
    \end{subfigure}%
    \caption{$f_{sgc}$, FP Reconstruction, BA graph model, noiseless}
...
\begin{figure*}%
    ...
    \end{subfigure}%
    \caption{$f_{sgc}$, LS Reconstruction, BA graph model, Noisy ($20dB$ signal)}
...
\begin{figure*}[h]%
    ...
    \end{subfigure}%
    \caption{$f_{sgc}$, LS Reconstruction, SBM graph model, noiseless}
...
We also see that under \bs{noiseless} LS reconstruction (Figs. \ref{subfig:BA_noiseless_simple_LS}, \ref{subfig:BA_noiseless_sgc_LS} and \ref{subfig:SBM_noiseless_sgc_LS}), our classification loss looks like $x \to \arccos\left(\sqrt{x}\right)$, which follows the theoretical interpretation in \bs{\ref{sec:Gen_Theory}}.
...
For CoRA, we find reconstruction-optimal sampling is the best for reconstruction and the worst for classification, with our method showing significant improvement over both random and reconstruction-optimal sampling. This experiment shows that even though we may have assumed binary classification and independent features to compute our sampling scheme, this does not reduce its applicability to similar downstream tasks like multi-class classification.
\end{mccorrection}
