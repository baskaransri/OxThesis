...
\begin{itemize}
    ...
    \item A linear reconstruction $\hat{\matr{X}}$ of the feature matrix, such as LS or FP, giving $\hat{\matr{X}} = \matr{R}_{\set{S}} \msubgen[\set{S},\set{D}]{\matr{X} + \sigma \cdot \matr{\epsilon}}$.
...
\label{thm:general_class_err}
    ...
    \begin{align}
        ...
        \\  &\text{Reconstruction Loss}  =  d \sum_{i \in \set{V}} \left(\phi_{i}(\matr{I})\right)^{2} + \left(\nu_{i}(\matr{I})\right)^{2} - 2c_{i}(\matr{I}) \label{eq:gen_rec_err}
...
\begin{proof}
    ...
    \begin{align}
     ...
     &= \sum_{i \in \set{V}}d \text{Var}(\matr{X}_{i}) + d\text{Var}(\hat{\matr{X}}_{i}) - 2d\text{Cov}(\hat{\matr{X}}_{i}, \matr{X}_{i}).
    \end{align}
   As $\expect{\matr{X}} = \expect{\hat{\matr{X}}} = 0$, this expands to (\ref{eq:gen_rec_err}).
...
%We can use \bs{Theorem \ref{thm:general_class_err}} to calculate the classification \bs{loss} for an $r$-layer \bs{linearized} GCN without bias, setting $\matr{G} = (\tilde{\matr{A}}_{\gamma})^{r}$ and $\vect{w} = \prod \matr{W}_{i}$. The classification \bs{loss then} does not depend on the weights \bs{$\vect{w}$} -- if our linearized GCN is an accurate representation of the true function, then the classification \bs{loss} is not dependent on its weights, only its structure \bs{$\matr{G}$} (i.e. the number of layers). 
 ...
 To interpret Theorem \ref{thm:general_class_err}, we consider any $\matr{G}$ and LS reconstruction of a noise-free $k$-bandlimited signal; specifically $\matr{\Sigma} = \projbl$. In this case, $\sqrt{c_{i}(\matr{G})} = \nu_{i}(\matr{G})$ and the {\text{classification loss}} is $\sum_{i \in \set{V}} \frac{1}{\pi}\arccos{\left(\sqrt{\frac{c_{i}}{\phi_{i}}}\right)}$ where $0 \leq \frac{c_{i}}{\phi_{i}} \leq 1$. Thus, we expect the behaviour of classification \bs{loss} as it changes with sample size to look similar to $x \to \arccos{\left(\sqrt{x}\right)}$.
...
\Require $\sigma:$ Noise standard deviation,
\Require $f:\, \mathbb{R}^{N \times d} \to \mathbb{R}^{N}: $ linear function s.t. the target classifier is $\matr{X} \to \text{sign}(f(\matr{X}))$
\Require $\matr{G}, \vect{w}:$ Linear classifier parameters defining $f(\matr{X}) = \matr{G}\matr{X}\vect{w}$
...
\Require $\matr{C}:$ $\text{Cov}(\matr{X}\vect{w})(\sqnormvec{\vect{w}})^{-1}$; this is $\text{Cov}((\matr{X}^{T})_{1})$ if the columns of $\matr{X}$ are i.i.d.
...
We now discuss our work in relation to Gama et al. \cite{gama2020rethinking} and Chamon et al. \cite{chamon2017greedy}, the main works in the GSP literature focusing on downstream tasks in the context of sampling. We first survey their theoretical results. Gama et al. focus on designing efficient joint sampling and reconstruction schemes under a linear reconstruction task represented by a matrix $\matr{H}$. They seek to construct an optimal `sketching operator' $\matr{H}_{\set{S}}$ which, when applied to the noisy observations, most closely approximates $\matr{H}$ applied to the true signal. From \cite[Proposition 2]{gama2020rethinking}, we can view their sketching operator as a combination of applying $\matr{H}$ after applying a reconstruction operator which they express as $\matr{R}_{X}\matr{C}^{T}\left(\matr{C}(\matr{R}_{X} + \matr{R}_{w})\matr{C}^{T}\right)^{-1}$, which in our notation would be $\msubgen[\set{N},\set{S}]{\text{Cov}(\vect{x})}\msubgen[\set{S}]{\text{Cov}(\matr{x}) + \text{Cov}(\matr{w})}^{-1}$. We mirror this definition in Line \ref{alg:defn_H_S} of our algorithm.  This \cite[Proposition 2]{gama2020rethinking}, along with \cite[(17) \& (18)]{gama2020rethinking} can be seen a version of our Theorem \ref{thm:general_class_err} for linear downstream tasks. We note that their \cite[Proposition 2]{gama2020rethinking} capture

Chamon et al. \cite{chamon2017greedy}, on the other hand, focus on greedy sampling and submodularity; similarly to Gama et al., they focus on a setting jointly optimising sampling and reconstruction in light of the setting. While their theoretical results do incorporate a linear `transform', understanding the impact of this transform, or downstream task, is not the focus of their work, and so analysis in this direction is limited.
...
\bs{We draw features $\matr{X}$ as described in Section \ref{sec:problem_setting} with $\matr{\Sigma} = \projbl$, i.i.d. columns, bandwidth $k = \frac{N}{10}$ and noise variance $\sigma^{2}=10^{-3}$ so the SNR is $20dB$, like in \cite{bai2020fast}. }
...
\end{equation}
For each graph, we construct $f_{sgc}$ as a linearized GCN with randomly initialized weights and added self-loops ($\gamma=1)$. We choose our architecture to mimic the strong baselines in \cite{luo2024classic}; we keep to a single convolutional layer to minimise oversmoothing in the case that $\matr{\Sigma} = \projbl$, and extend to two layers in the case $\matr{\Sigma} = \matr{L}^{\dagger}$.
Specifically, we set
\begin{equation}
 f_{sgc}(\matr{X}) =  \begin{cases}
\left(\tilde{\matr{A}}_{1}\matr{X}\matr{W}_{1} \right)\matr{W}_{readout} &\text{if }\matr{\Sigma} = \projbl \\
\left(\tilde{\matr{A}}_{1}\left(\tilde{\matr{A}}_{1}\matr{X}\matr{W}_{1}\right)\matr{W}_{2} \right) \matr{W}_{readout} &\text{if }\matr{\Sigma} = \matr{L}^{\dagger} \\
\end{cases}\label{eq:SGC_structure}
\end{equation}
 where $\matr{W}_{1} \in \mathbb{R}^{64 \times 32}$, $\matr{W}_{2} \in \mathbb{R}^{32 \times 32}$,  $\matr{W}_{readout} \in \mathbb{R}^{32 \times 1}$ are all populated via Glorot initialization \cite{glorot2010understanding}.
...
We present a table of the settings of the following figures. We consider Figs. \ref{subfig:BA_noiseless_sgc_LS} \& \ref{subfig:BA_noiseless_sgc_LS_lpinv} as baseline experimental setups, and vary one parameter per figure. After this, we present our two real world experiments. We use \texttt{''} to indicate when a setting is the same as the previous figure
...
\begin{figure*}%
    ...
    \includegraphics[width=\columnwidth, trim={1000cm 2cm 0 0}, clip]{figures/proj2/plots/sgc-paper/BA_bl_LS_0_noise_500_nodes_1_layers_class_vs_rec.png}}
    \caption{Loss Comparison \\}
    \label{subsubfig:BA_noiseless_sgc_LS_comp_lpinv}
    \end{subfigure}
    \hfill
    \fi
    \begin{subfigure}[T]{0.33\columnwidth}
...
\begin{figure*}%
    ...
    \includegraphics[width=\columnwidth]{figures/proj2/plots/sgc-paper/BA_bl_feat_prop_0_noise_200_nodes_1_layers_class_vs_rec.png}}
    \caption{Loss Comparison \\}
    \label{subsubfig:BA_noiseless_sgc_feat_prop_comp_lpinv}
    \end{subfigure}
    \hfill
    \fi
    \begin{subfigure}[T]{0.33\columnwidth}
...
\begin{figure*}%
    ...
    \includegraphics[width=\columnwidth]{figures/proj2/plots/sgc-paper/BA_bl_LS_0.03162277660168379_noise_500_nodes_1_layers_class_vs_rec.png}}
    \caption{Loss Comparison}
    \label{subsubfig:BA_noisy_sgc_LS_comp_lpinv}
    \end{subfigure}
    \hfill
    \fi
    \begin{subfigure}[T]{0.33\columnwidth}
...
\begin{figure*}[h]%
    ...
    \includegraphics[width=\columnwidth]{figures/proj2/plots/sgc-paper/SBM_bl_LS_0_noise_500_nodes_1_layers_class_vs_rec.png}}
    \caption{Loss Comparison \\}
    \label{subsubfig:SBM_noiseless_sgc_LS_comp_lpinv}
    \end{subfigure}
    \fi
    %\hfill
    \begin{subfigure}[T]{0.3\columnwidth}
...
In our real-world fMRI dataset (Fig. \ref{fig:fmri}) we find, for classification loss, both reconstruction and classification-optimal sampling outperforming random sampling, with our novel classification-optimal sampling scheme marginally outperforming at sample sizes over 90.

For CoRA, we find reconstruction-optimal sampling is the best for reconstruction and the worst for classification, with our method showing significant improvement over both random and reconstruction-optimal sampling. This experiment shows that even though we may have assumed binary classification and independent features to compute our sampling scheme, it can also be applied to similar downstream tasks like multi-class classification.
