...
\subsubsection{\mccorrect{Relationship to Double Descent}}
{\color{blue}The impact of sample size on reconstruction error has also been studied in the literature on double descent, by considering the ratio of data to the number of parameters as a complexity measure. We discuss results in both the broader double descent literature and the Graph Machine Learning double descent literature.

In the broader, non-graph literature on double descent, Hastie et al \cite{hastie2022surprises} and Nakkiran et al \cite{nakkiran2019more} show how more data can hurt in linear regression. They both study test risk, which can be made equivalent to our MMSE objective, and show asymptotically \cite[Theorem 1]{hastie2022surprises}, \cite[Claim 1]{nakkiran2019more} and non-asymptotically \cite[Theorem 2]{hastie2022surprises} that the test risk has a $\Lambda$-shape as sample size varies, i.e. that asymptotically at sufficiently low SNRs, linear regression is non-monotonic in sample size, with a peak in error as the number of samples approaches the number of parameters from either side. This is illustrated experimentally in \cite[Fig. 1]{nakkiran2019more}.

However, these results do not immediately carry over to the GSP setting. While the natural way to extend these statements to the graph setting would be to let $x_{i}$ be uniformly drawn from the rows of $\matrsubU{N}$, such an approach would have three limitations; firstly, it would require theorems which characterised the distribution of the entries for Laplacian eigenvectors for a graph model, which are limited -- while Tang et al \cite{tang2018limit} have results for random dot product graphs (RPDG) like Erdős–Rényi and Stochastic Blockmodel graphs, such results are not available for graph models such as the Barabasi-Albert graph. Secondly, because of the concentration inequality methods used in the paper, it could only directly apply to graph models whose eigenvector entries Gaussian -- it may not be possible to use such methods to study non-RPDG graph models, such as the Barabasi-Albert graph model, or indeed any fixed graph. Finally, it would not obviously generalise to other reconstruction methods, such as GLR reconstruction.

In the graph machine learning literature on double descent, 
Shi et al \cite{shi2024homophily} extend the classical double descent literature to GNNs on a Contextual Stochastic Blockmodel, considering the ratio of the number of labels to the number of nodes as a complexity measure; they show that in this transductive setting that double descent occurs, and that test risk is non-monotonic in label ratio, and discuss how homophily modulates this relationship. Duranthon et al \cite{duranthon2024asymptotic} build on Shi, extending to arbitrary convex loss and giving exact asymptotics. These present a thorough theoretical investigation of the impact of label ratios and feature ratios on test risk on the Contextual Stochastic Blockmodel under random sampling, while leaving non-Stochastic Blockmodels graph models and non-uniform sampling to future work.

While the double descent literature give thorough and well-reasoned demonstrations and proofs of non-monotonicity, there is still work to be done to establish non-monotonicity of test risk with sample size in the GSP graph sampling setting under different sample selection schemes.}
