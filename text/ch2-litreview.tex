...
We now introduce the most common signal model used in the graph signal processing literature, the bandlimited signal model. A $k$-\emph{bandlimited signal} is a linear combination of the first $k$ columns of $\matr{U}$\cite{EOptimalChen}. To construct these signals, we define $\projbl$ as the projection operator that $k$-bandlimits a signal:
\begin{equation} 
    \projbl = \matrsubU{N}\matrsubU{N}^{T}.
\end{equation}
...
Now that we have established potential signal models, we can discuss handling missing data. In this thesis, we focus on the approach of reconstructing data to handle missing data, which also known as inpainting. We begin with an intuitive example.
...
We now briefly cover the GCN architecture, one of the original GNNs, and SGC, a linearisation version of the GCN.
