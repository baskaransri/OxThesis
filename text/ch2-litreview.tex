...
Now that we have established potential signal models, we can discuss handling missing data. Data may be missing because of x,y,z reasons (sampling or just missing or adversarial); while there is literature on optimising sampling, we focus purely on reconstruction in this chapter. In this thesis, we focus on the approach of reconstructing data to handle missing data, which also known as inpainting. We begin with an intuitive example.
...
\end{flalign}
\mccorrect{GLR can be seen to be a form of Tikhonov regularisation} \cite{belkin2004tikhonov}.
...
\mccorrect{Pukelsheim derives A-, D- and E-Optimality criteria from the theory of matrix means. Pukelshiem also notes that matrix mean theory also implies T-Optimality, maximising} $\text{tr}(\matr{P})$, \mccorrect{which has been employed approximately in the GSP setting in} \cite{puy2018random}. \mccorrect{However, this optimality criterion does not translate to a natural objective in the way we have described above for A-, D- and E-Optimality -- Pukelsheim describes T-Optimality as `rather meaningless'}\cite[Section 6.5]{pukelsheim2006optimal} \mccorrect{and lists ways in which its `weakness' causes a list of `pathologies', making discussion of the theory of sampling more difficult with little gain. Therefore, we exclude T-Optimality from our theoretical analysis. }
...
Of the four main theoretical results on the impact of sample size in the literature, three focus on slightly different settings: Shomorony et al. \cite{shomorony2014sampling} present sample size bounds for perfect signal reconstruction in the noiseless setting, while Chamon et al. \cite[Lemma 1]{chamon2017greedy} prove that MSE decreases as sample size increases in the noisy setting and provides bounds on the impact of sample size on MSE, but assumes a specific form of regularised LS which is Bayesian optimal, and Belkin et al. \cite{belkin2004tikhonov} provide the only theoretical result on GLR, using learning theory to derive bounds on generalisation risk for a constrained variant of GLR which can only produce signals $\vect{x}$ where $\vect{1}^{T}\vect{x} = 0$. Finally, Tremblay \cite{tremblay2024graph} studies determinental sampling, which samples proportional to the determinant of a submatrix of a `kernel'. Typically, these sampling schemes do not provide a fixed sample size, instead sampling over all possible sample sizes proportional to said determinant. Tremblay provides a bound \cite{tremblay2024graph}[(4.14)] on the expectation of MSE under said sample scheme; however, he also defines fixed-sample size determinental sampling schemes and results from the wider DPP sampling theory\fi

We now discuss the GLR reconstruction setting. Theoretical work on GLR reconstruction is scarce in the GSP literature; GLR is generally considered in noisy scenarios as the implicit signal model of GLR is a multivariate gaussian signal with covariance $\matr{L}^{\dagger}$ \cite{dong2016learning}, which does not match with the usual bandlimited signal model.  To construct an Optimal Bayesian Reconstruction variant of GLR would potentially involve studying LapRLS \cite[Section 4.2]{belkin2006manifold}, and studies on LapRLS are scarce in the GSP literature; Finally, GLR reconstruction does not immediately lend itself to being structured in the form $\text{tr}(\matr{P})$, making analysis under determinental sampling much less immediate.
