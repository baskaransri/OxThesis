...
We provide the definitions and a theoretical result to quantify this. %\mccorrect{We only consider simplifications which compare an observed set $\set{S}$ to its subset $\set{T} \subset \set{S}$ which is reflected in our definitions.}
...
\end{table}
\end{lemma}
\begin{proof}[Proof Sketch] \mccorrect{We first show that under LS,} $\Delta_{1}(\set{S},v)$ can only be $-1$ or $0$. \mccorrect{We then use a spectral argument via Cauchy's Interlacing Theorem for each case to show whether} $\Delta_{2}$ \mccorrect{is increasing or decreasing. See} Appendix \ref{app:table_delta_proof} for a full proof.
\end{proof}
...
\section{Conclusion}
In this Chapter we studied the impact of sample size on linear reconstruction of noisy graph signals. We showed theoretically and experimentally, in the same settings as much of the sample set selection literature, that reconstruction error is not always monotonic in sample size, i.e., at sufficiently low SNRs, reconstruction error can sometimes be improved by \emph{reducing} sample size. %, even if the sample set was picked by a greedy optimal sampling scheme given a fixed sample size.
