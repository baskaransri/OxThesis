...
\begin{itemize}
    ...
    \end{itemize}
    \item Linear reconstruction, in particular LS.
\end{itemize}
...
\noindent We briefly give a symbolic definition of our signal and noise model. We assume we observe a corrupted signal $\vect{y} = \vect{x} + \vect{n}$ %at a subset of the vertices $\set{S}$ 
where
\begin{itemize}
...
We start by looking at possible combinations of signs of $\Delta_{1}(\set{S},v)$ and $\Delta_{2}(\set{S},v)$, which we present in Table \ref{tbl:LS_Reconstruction_Delta}.
