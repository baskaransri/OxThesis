...
``Where is the knowledge we have lost in information?''
  \qauthor{--- T. S. Eliot, \emph{Choruses from ``The Rock''}}
\end{savequote}
...
In this Chapter we study how changing sample size impact reconstruction performance, giving a characterisation of when decreasing sample size decreases reconstruction error can happen, and proving cases when it must happen under LS. 
...
Graph signal processing (GSP) generalises the highly successful tools of sampling and reconstruction in classical signal processing, \iffalse by extending the classical shift operator to a graph shift operator \cite{ortega2018graph} such as the adjacency matrix \cite{EOptimalChen} or the graph Laplacian\fi enabling us to extrapolate the full data across the graph from observations on a subset of vertices \cite{tanaka2020sampling}. %or summaries of the data \cite{tanaka2020sampling}.
...
The literature studies the impact of sample size both empirically and theoretically, as summarised in Chapter \ref{ch:2-litreview}, Table \ref{tbl:Lit_review}. As per this summary, we note that the theoretical results present in the literature do not cover the full breadth of settings studied empirically, for example, requiring noiselessness \cite{shomorony2014sampling} or requiring reconstruction with a specific form of regularised LS \cite{chamon2017greedy}, Optimal Bayesian Reconstruction, which is Bayesian-optimal and requires exact knowledge of noise levels. \mccorrect{In this chapter we only consider `ridgeless' min-norm Least Squares -- we note explicitly that it is known from the results on Optimal Bayesian Reconstruction}\cite{chamon2017greedy} \mccorrect{means that ridge regression with a correctly tuned penalty parameter, whether through knowledge of noise (Optimal Bayesian Reconstruction) or via empirical methods such as cross-validation, will have monotonically deceasing error with sample size.}
...
In this chapter, we provide a theoretical characterisation of the impact of sample size on the average MSE (or MMSE  -- see Chapter \ref{ch:2-litreview}, Section \ref{sec:thesis_define_MMSE}) across the full range of possible sample sizes in some of the most common settings, i.e. noisy observations and unregularised LS  reconstruction, filling some of the gap in the literature described in Chapter \ref{ch:2-litreview}, Section \ref{sec:thesis_lit_review}. We focus on general theory applicable to any linear reconstruction method, then specialise to least squares (LS) reconstruction for our existence and asymptotic results, leaving more challenging reconstruction settings such as graph Laplacian regularisation (GLR) to the next chapter. More specifically, we focus on {\color{blue}extending the investigations of Hastie et al \cite{hastie2022surprises} and Nakkrian et al \cite{nakkiran2019more} to the GSP and graph settings --} whether under sufficiently low SNRs, decreasing sample size may actually decrease MSE. Furthermore, we investigate both the full range of sample sizes and all possible levels of noise, which is important for the application of graph sampling to domains where the noise may be greater than the signal (e.g. finance, with typically low signal-to-noise ratio \cite{nabar2023conservative}, or environmental sciences,  with inaccurate measurements from low-cost sensors \cite{bush2023impact}) or when observing more samples might not be feasible (e.g. resource-constrained settings). This breadth is only possible through our rigorous theoretical characterisation which allows us to understand behaviour at high noise levels, to characterise behaviour on arbitrarily large graphs, and to show when certain behaviours of MSE happen and why. %While theorems often need to be narrow in scope to allow for tractable proofs, our results are notable in how loose their restrictions are; our results for LS reconstruction apply to all graphs and every determinstic sampling scheme in the literature, while the conditions of our main result for GLR reconstruction under full-band noise, empirically, hold for every Erdős–Rényi, Stochastic Blockmodel and Barabasi-Albert graph tested (Table \ref{tbl:empirical_probabilities_conditions}).

Our results begin by using a Bias-Variance decomposition, {\color{blue} like \cite{hastie2022surprises} and \cite{nakkiran2019more}}, to explain why decreasing sample size may decrease MSE for any linear reconstruction method (Section \ref{sec:general}), and we then specialise (Section \ref{sec:every_x}) to LS to show the existence of conditions under which decreasing sample size \emph{will} decrease sample size for this unregularised reconstruction method (Section \ref{sec:LS_full_band}). We leave the more challenging setting of regularised reconstruction to Chapter \ref{ch:proj1_GLR}. We prove that under LS reconstruction of a $k$-bandlimited signal, if the samples were chosen to be optimal in the noiseless case then we can always reduce MSE under high noise by reducing sample size from $k$ to $k-1$ {\color{black}(Section \ref{sec:LS_full_band}, Theorem \ref{thm:noiseless_optimality_means_noise_sensitivity})}. Our experiments (Section \ref{sec:experiments_LS}) validate this for large Stochastic Blockmodel and Barabasi-Albert graphs (Section \ref{sec:LS:experiments_synthetic}), demonstrating the applicability of our results to graphs with community structure and large scale-free networks. We further validate our theorems on real world graphs (Section \ref{sec:LS:experiments_real}). We also investigate how sensitive our results are to different kinds of noise by presenting variants of our theoretical and empirical results under both full-band (Section \ref{sec:LS_full_band}) and bandlimited (Section \ref{sec:LS_bandlimited}) noise. Although our results are mainly of theoretical nature, they may provide useful guidance on choosing the appropriate sample size in light of noise levels in the data.

\iffalse
This presents four primary contributions: 
\begin{enumerate}
    \item A theoretical characterisation of how decreasing sample size may decrease MSE under LS,
    \item Analysis of LS under bandlimited noise to show non-monotonicity of the MSE is not caused by just the high frequency component of the noise.
    \item Asymptotic analysis, showing how the non-monotonicity of the MSE with sample size persists as $N \to \infty$.
    \item Extensive experimental simulations illustrating the theoretical results under LS, for both full-band and bandlimited noise.
\end{enumerate}
\fi
\iffalse
...
In this Chapter, we are interested in a theoretical characterisation of the impact of sample size on MSE under all possible SNRs. 
...
\begin{itemize}
    ...
    \end{itemize}
    \item Linear reconstruction, in particular LS (as defined in \eqref{eq:LS_Defn}).
\end{itemize}

In this chapter, though our experiments all use the combinatorial Laplacian, our theoretical results apply to any positive semi-definite shift operator (including the symmetrically normalized Laplacian).


We briefly give a symbolic definition of our signal and noise model. We assume we observe a corrupted signal $\vect{y} = \vect{x} + \vect{n}$ %at a subset of the vertices $\set{S}$ 
where
\begin{itemize}
...
\end{itemize}
Our results apply to any noise with these properties (e.g. Gaussian or Rademacher noise).
We refer to the $\text{Cov}(\vect{\epsilon}) = \matr{I}_{N}$ case as `full-band noise' as the associated corrupted signal $\vect{y}$ has high frequency components, whereas the other case does not. %In the literature, noise levels are often described using the SNR = $\frac{\expect{\sqnormvec{ \vect{x} }}}{\expect{\sqnormvec{\vect{n}}}}$,  a ratio of norms which must be positive\footnote{It is common in the literature to express the SNR in decibels, which may be negative, while its ratio form remains positive. We will use the ratio form unless otherwise noted, so for example $-20dB$ will be written as $10^{-20/10}  > 0$.}. 
We note that under full-band noise we have $\sigma^{2} = \frac{k}{N \cdot \text{SNR}}$ and under $k$-bandlimited noise we have $\sigma^{2} = \frac{1}{\text{SNR}}$.


In this Chapter, we study the behaviour of the MMSE criterion, that is, the MSE averaged over a known signal model and known noise model, which we write as
\begin{equation}
...
The focus of this chapter is not to characterise all the cases where decreasing sample size decreases MSE, but rather to clearly show that it does happen in a wide variety of cases. In service of this, we focus on certain broad cases that are more tractable, which we call `simplifications'. For example, we only consider compare a sample set $\set{S}$ to a subset of it, i.e., $\set{T} \subset \set{S}$, rather than comparing any smaller set to any larger set.

Our general approach for a reconstruction method and noise type is described in Figure \ref{fig:theory_structure_flowchart}:
\begin{figure}[htbp]
    \centering
    \input{figures/proj1/tikz/reconstruction_flowchart.tikz}
    \caption{General approach for our theoretical analysis}
    \label{fig:theory_structure_flowchart}
\end{figure}
For LS, we simplify the problem by only considering decreasing the sample size by one at a time, which we call the `single vertex' simplification. We pay particular attention to subsets sampled by sampling schemes that are optimal in the noiseless setting (Subsection \ref{sec:LS_full_band}). 
We consider reconstruction both under full-band noise (Section \ref{sec:LS_full_band}) and bandlimited noise (Subsection \ref{sec:LS_bandlimited}) to show that the reduction in MSE from reducing sample size is not sensitive to our noise model, nor due to the high frequency component of the noise.
An overview of the four steps in our general approach together with the main results are summarised in Table \ref{tbl:general_theory_LS}.
...
We note that given Theorem \ref{main_general} and these terms, for any given reconstruction method and two sets $\set{S} \supset \set{T}$ we can now analytically compute if a given $\set{T}$ is better than a set $\set{S}$ under our signal model, and at what noise levels. The rest of the chapter focuses on finding such sets, by specialising to a specific reconstruction method.
...
In this subsection we show how decreasing sample size by one can decrease MSE under LS and full-band noise. 

%From Table \ref{tbl:LS_Reconstruction_Delta}, we see that reducing sample size never reduces bias under LS, giving further reason on top of Proposition \ref{propn:averages_generalise_to_forall} to focus on when reducing sample size reduces sensitivity-to-noise.  %We use the `single vertex' simplification as Table \ref{tbl:LS_Reconstruction_Delta} lets us neatly categorise the sign of $\Delta_{2}(\set{S},v)$.

\subsubsection{\bs{Overview and Simplification}}
Our approach in this subsection is as follows. We consider the `single vertex' simplification where $\set{T} = \{v\}$. For a sample set $\set{S}$ and $v\in\set{S}$, we first characterise under exactly what conditions $\set{S} \backslash \{v\}$ is better than $\set{S}$ (Corollary \ref{main_ls}). We then show that the conditions must occur under sampling schemes which are optimal in the noiseless case (Theorem \ref{thm:noiseless_optimality_means_noise_sensitivity}). Finally, we comment on how the conditions persist as $N \to \infty$.

We define some shorthands for use with the `single vertex' simplification. For a vertex $v \in \set{S}$ we write:
\begin{align}
...
We start by looking at possible combinations of signs of $\Delta_{1}(\set{S},v)$ and $\Delta_{2}(\set{S},v)$, which we present in the following Lemma. %Table \ref{tbl:LS_Reconstruction_Delta}.
\begin{lemma}
\label{lemma:LS_sign_table}
Under LS and full-band noise, only the combinations of signs of $\Delta_{1}(\set{S},v)$ and $\Delta_{2}(\set{S},v)$ marked with a $\checkmark$ in Table \ref{tbl:LS_Reconstruction_Delta} are possible.%; combinations of signs marked with a $\times$ in Table \ref{tbl:LS_Reconstruction_Delta} are impossible.
\begin{table}[h]
\centering
        \begin{tabular}{lcc}
        \toprule
        ...
        \bottomrule
       \end{tabular}
\caption{Possible signs of $\Delta_{1}(\set{S},v)$ and $\Delta_{2}(\set{S},v)$ under LS and full-band noise}
       \label{tbl:LS_Reconstruction_Delta}
\end{table}
\end{lemma}
\begin{proof}[Proof Sketch] {We first show that under LS,} $\Delta_{1}(\set{S},v)$ can only be $-1$ or $0$. {We then use a spectral argument via Cauchy's Interlacing Theorem for each case to show whether} $\Delta_{2}$ {is increasing or decreasing. See} Appendix \ref{app:table_delta_proof} for a full proof.
\end{proof}
%\noindent In Table \ref{tbl:LS_Reconstruction_Delta}, $\checkmark$ can happen and $\times$ cannot happen. A more detailed explanation of the table and proofs of the `$\times$' subcases are presented in Appendix \ref{app:table_delta_proof}. %We now discuss LS and GLR separately in the following sections.
...
\label{main_ls}
 ...
 \end{equation}
 \end{corollary}
\begin{proof}[Proof Sketch]  Under LS, $\Delta_{1}(\set{S},v)$ can only be $-1$ or $0$, which simplifies $\tau$ in Theorem \ref{main_general} to (\ref{eq:main_ls_simplified_tau}). Now consider the two cases:
\begin{description}[leftmargin=!, labelwidth=\widthof{{If $\Delta_{1}(\set{S},v) = -1$:}}]
\item[{If $\Delta_{1}(\set{S},v) = 0$:}] 
...
\end{description}
See Appendix \ref{proof_appendix_LS} for a full proof.
\end{proof}
...
\mccorrect{We provide a less rigorous, higher level discussion on how non-monotonicity of MMSE under Least Squares with sample size relates to graph structure. We have proven that MMSE at a sufficiently high noise is $\Lambda$-shaped for \emph{any} connected graph with distinct eigenvalues, regardless of graph structure, which means that it is impossible for graph structure to completely mitigate non-monotonicity. Secondly, we note that sampling schemes often depend on graph structure -- indeed determinental sampling can be shown to have an MSE which doesn't depend on the graph structure at all, which can be seen from} \cite[(4.14)]{tremblay2024graph}\mccorrect{combined with comments made in our introduction chapter. This means that we cannot make statements about the impact of graph structure on non-montonicity without picking a specific sampling scheme, as no statement can hold in general.

When we restrict ourselves to uniform random sampling, we can consider that the best possible $\matrsubU{S}$ would be the identity, which would correspond to the completely disconnected graph. This suggests that less connected graphs would have greater robustness to non-monotonicity patterns. Furthermore, we know that the non-monotonicity pattern in Hastie et al} \cite{hastie2022surprises} \mccorrect{corresponds to gaussian eigenvector entries; this suggests more structured graphs drawn from graph models such as graphons, rather than graphs drawn from random dot product graph models, such as ER or SBM graph models, would be more robust to non-monotonicity. }

\section{Experiments}
\label{sec:experiments_LS}
...
We first demonstrate the applicability of our results with plots of the thresholds $\tau(\set{S},v)$ against sample size (Figs. \ref{LS_SNR_Threshold_plots_all} - \ref{LS_SNR_Threshold_plots_big_BA}). These plots show concrete SNR values for the thresholds, giving a practical understanding of how high noise levels need to be for reducing sample size to reduce MSE for different random graph models and parameters.  We then demonstrate the validity of our results by plotting $\textrm{MSE}_{\set{S}}$ against sample size (Figs. \ref{LS_ER_MSE_fig} - \ref{LS_BA_MSE_bandlimited_fig}) at SNRs below, near and above the derived thresholds, showing that the behaviour of $\textrm{MSE}_{\set{S}}$ follows our theoretical results. We finally present similar results on real-world datasets (Figs. \ref{LS_SNR_Threshold_plots_all_real} \& \ref{LS_real_MSE_fig}), validating the applicability of our results to real world datasets.
...
\begin{itemize}
    ...
    %\item Ring (Ring).
\end{itemize}
We consider 10 instantiations of each model for the plots. We present threshold plots for graphs with 500, 1000, 2000 and 3000 vertices. We only present MSE plots for graphs with 500 vertices  (like \cite[Fig 8]{bai2020fast}) as they are intended as an accompaniment to our threshold plots and theorems to demonstrate their validity, and a single graph size suffices. 
...
\label{sec:LS:experiments_synthetic}
We present threshold plots and MSE plots for all of our graph models.We present Table \ref{tbl:theory_experiment_correspondence_LS} showing how our plots correspond to our theoretical results, which also corresponds to the summary of theoretical results in Table \ref{tbl:general_theory_LS}.
...
\begin{figure*}%
    \centering
    \begin{subfigure}{0.32\textwidth}
    \includegraphics[width=\linewidth]{figures/proj1/plots/LS_threshold/SBM_1000_bandwidth_100_thresholds_LS.png}
    \caption{1000 vertices} 
    \label{snr_SBM_1000}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.32\textwidth}
    \includegraphics[width=\linewidth]{figures/proj1/plots/LS_threshold/SBM_2000_bandwidth_200_thresholds_LS.png}%
    \caption{2000 vertices}%
    \label{snr_SBM_2000}%
    \end{subfigure}
    \hfill%
    \begin{subfigure}{0.32\textwidth}
    \includegraphics[width=\linewidth]{figures/proj1/plots/LS_threshold/SBM_3000_bandwidth_300_thresholds_LS.png}%
...
\begin{figure*}%
    \centering
    \begin{subfigure}{0.32\textwidth}
    \includegraphics[width=\linewidth]{figures/proj1/plots/LS_threshold/BA_3_1000_bandwidth_100_thresholds_LS.png}
    \caption{1000 vertices} 
    \label{snr_BA_1000}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.32\textwidth}
    \includegraphics[width=\linewidth]{figures/proj1/plots/LS_threshold/BA_3_2000_bandwidth_200_thresholds_LS.png}%
    \caption{2000 vertices}%
    \label{snr_BA_2000}%
    \end{subfigure}
    \hfill%
    \begin{subfigure}{0.32\textwidth}
    \includegraphics[width=\linewidth]{figures/proj1/plots/LS_threshold/BA_3_3000_bandwidth_300_thresholds_LS.png}%
    \caption{3000 vertices}%
    \label{snr_BA_3000}%
    \end{subfigure}%
...
\begin{figure*}%
    \centering
    \begin{subfigure}{0.32\textwidth}
    \includegraphics[width=\linewidth]{figures/proj1/plots/LS_MSE/SBM_500_bandwidth_50_SNRdbs_-10.0_samps_100_MSE_LS.png}
    \caption{SNR = $10^{-1}$}
    \label{SBM_MSE_subfiga}
    \end{subfigure}\hfill
    \begin{subfigure}{0.32\textwidth}
    \includegraphics[width=\linewidth]{figures/proj1/plots/LS_MSE/SBM_500_bandwidth_50_SNRdbs_20.0_samps_100_MSE_LS.png}%
    \caption{SNR = $10^{2}$}%
    \label{SBM_MSE_subfigb}%
    \end{subfigure}\hfill%
    \begin{subfigure}{0.32\textwidth}
    \includegraphics[width=\linewidth]{figures/proj1/plots/LS_MSE/SBM_500_bandwidth_50_SNRdbs_100.0_samps_100_MSE_LS.png}%
    \caption{SNR = $10^{10}$}%
...
\begin{figure*}%
    \centering
    \begin{subfigure}{0.32\textwidth}
    \includegraphics[width=\linewidth]{figures/proj1/plots/LS_MSE/BA_3_500_bandwidth_50_SNRdbs_-10.0_samps_100_MSE_LS.png}
    \caption{SNR = $10^{-1}$}
    \label{BA_MSE_subfiga}
    \end{subfigure}\hfill
    \begin{subfigure}{0.32\textwidth}
    \includegraphics[width=\linewidth]{figures/proj1/plots/LS_MSE/BA_3_500_bandwidth_50_SNRdbs_20.0_samps_100_MSE_LS.png}%
    \caption{SNR = $10^{2}$}%
    \label{BA_MSE_subfigb}%
    \end{subfigure}\hfill%
    \begin{subfigure}{0.32\textwidth}
    \includegraphics[width=\linewidth]{figures/proj1/plots/LS_MSE/BA_3_500_bandwidth_50_SNRdbs_100.0_samps_100_MSE_LS.png}%
    \caption{SNR = $10^{10}$}%
...
\begin{figure*}%
    \centering
    \begin{subfigure}{0.32\textwidth}
    \includegraphics[width=\linewidth]{figures/proj1/LS_MSE_bl_old/ER_0pt8_500_bandwidth_50_SNRdbs_-10.0_samps_100_MSE_LS.png}
    \caption{Bandlimited noise, SNR = $10^{-1}$}
    \label{bandlimited_MSE_subfiga}
    \end{subfigure}\hfill
    \begin{subfigure}{0.32\textwidth}
    \includegraphics[width=\linewidth]{figures/proj1/plots/LS_MSE/ER_0pt8_500_bandwidth_50_SNRdbs_0.0_samps_100_bl_MSE_LS.png}
    \caption{Bandlimited noise, SNR = $1$}%
    \label{bandlimited_MSE_subfigb}%
    \end{subfigure}\hfill%
    \begin{subfigure}{0.32\textwidth}
    \includegraphics[width=\linewidth]{figures/proj1/LS_MSE_bl_old/ER_0pt8_500_bandwidth_50_SNRdbs_100.0_samps_100_MSE_LS.png}
...
\begin{figure*}%
    \centering
    \begin{subfigure}{0.32\textwidth}
    \includegraphics[width=\linewidth]{figures/proj1/thesis-plots/LS_MSE/SBM_50x10n_0pt1intra_0pt7inter_500_bandwidth_50_SNRdbs_-10.0_samps_100_bl_MSE_LS.png}
    \caption{SNR = $10^{-1}$}
    \label{bandlimited_SBM_MSE_subfiga}
    \end{subfigure}\hfill
    \begin{subfigure}{0.32\textwidth}
    \includegraphics[width=\linewidth]{figures/proj1/thesis-plots/LS_MSE/SBM_50x10n_0pt1intra_0pt7inter_500_bandwidth_50_SNRdbs_0.0_samps_100_bl_MSE_LS.png}
    \caption{SNR = $1$}%
    \label{bandlimited_SBM_MSE_subfigb}%
    \end{subfigure}\hfill%
    \begin{subfigure}{0.32\textwidth}
    \includegraphics[width=\linewidth]{figures/proj1/thesis-plots/LS_MSE/SBM_50x10n_0pt1intra_0pt7inter_500_bandwidth_50_SNRdbs_100.0_samps_100_bl_MSE_LS.png}
    \caption{SNR = $10^{10}$}%
...
\begin{figure*}%
    \centering
    \begin{subfigure}{0.32\textwidth}
    \includegraphics[width=\linewidth]{figures/proj1/thesis-plots/LS_MSE/BA_3_500_bandwidth_50_SNRdbs_-10.0_samps_100_bl_MSE_LS.png}
    \caption{SNR = $10^{-1}$}
    \label{bandlimited_BA_MSE_subfiga}
    \end{subfigure}\hfill
    \begin{subfigure}{0.32\textwidth}
    \includegraphics[width=\linewidth]{figures/proj1/thesis-plots/LS_MSE/BA_3_500_bandwidth_50_SNRdbs_0.0_samps_100_bl_MSE_LS.png}%{figures/proj1/plots/LS_MSE/BA_3_500_bandwidth_50_SNRdbs_0.0_samps_100_bl_MSE_LS.png}
    \caption{SNR = $1$}%
    \label{bandlimited_BA_MSE_subfigb}%
    \end{subfigure}\hfill%
    \begin{subfigure}{0.32\textwidth}
    \includegraphics[width=\linewidth]{figures/proj1/thesis-plots/LS_MSE/BA_3_500_bandwidth_50_SNRdbs_100.0_samps_100_bl_MSE_LS.png}
    \caption{SNR = $10^{10}$}%
...
\subsubsection{$\tau$ plots -- full-band noise}
...
\subsubsection{$\tau$ plots - bandlimited noise}
We do not provide plots for this case, as our theoretical results provide the magnitude of $\tau$: by Corollary \ref {corr:LS_bandlimited_noise_sample_only_k}, $\tau(\set{S},v) = 1$ for $|\set{S}| \leq k$ under sequential noiseless-optimal sampling. Corollary \ref{corr:LS_bandlimited_noise_sample_only_k} overall says something stronger, i.e. we can always reduce MSE by reducing sample size if $\text{SNR} < 1$ at any sample size.
...
\label{sec:LS:experiments_real}

%%% REAL DATASET experiments



We first discuss the plots of $\tau$ for real world datasets. In practice, one cannot know for sure what the signal model of a real world signal is; however, computation of $\tau$ is dependent on our choice of theoretical signal model. We have therefore computed $\tau$ under the signal model assumptions given in Section \ref{sec:signal_model}, assuming full-band noise.
...
We validate the actual value of $\tau$ with MSE experiments (Fig. \ref{LS_real_MSE_fig}).
...
\section{Conclusion}
In this Chapter we studied the impact of sample size on linear reconstruction of noisy graph signals. We showed theoretically and experimentally, in the same settings as much of the sample set selection literature, that reconstruction error is not always monotonic in sample size, i.e., at sufficiently low SNRs, reconstruction error can sometimes be improved by \emph{reducing} sample size, {\color{blue} extending non-monontonicity results from the double descent setting to the GSP sampling setting}. %, even if the sample set was picked by a greedy optimal sampling scheme given a fixed sample size.
...
%For example, the limitation of ordinary LS reconstruction may be mitigated by regularisation schemes such as that proposed in \cite{chamon2017greedy}.
...
Another use case could be a practical sampling algorithm. %for both the LS and GLR reconstructions.
For LS reconstruction, if one has chosen $k$ or fewer nodes by some noiseless-optimal scheme, one can calculate $\tau(\set{S},v)$ for each element of $\set{S}$, remove $v$ if $\text{SNR} < \tau(\set{S},v)$, and repeat until it is no longer possible to find a node $v$ in our remaining set $\set{S}_{rem}$ where $\text{SNR} < \tau(\set{S}_{rem},v)$.

{\color{blue} A final recommendation would be to use optimally tuned ridge regression (`Optimal Bayesian Reconstruction'), tuning the parameter either from knowledge of the noise or via empirical methods such as CV -- it is proven that the finding that ``more data can hurt'' does not hold with optimal parameter choice.}
